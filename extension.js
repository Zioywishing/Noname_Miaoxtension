...
if(get.type(card)=='basic' && get.type(card)=='trick')   flag=  true;
					...
					"zioy_biubiubiu": {
                        ...
                        forced:true,
                        filter:function(event,player){
                            return (get.type(event.card)=='trick'&&event.card.isCard)&&event.card.name != 'shandian'&&event.player.getExpansions('zioy_biubiubiu').length==0&&player.storage.zioy_biubiubiu_flag1;
                        },
                        init:function(player){
                        ...
                        subSkill:{
                            uctt:{
                                ...
                                filter:function(event,player){
                                    ...
                                    }) > 0 && player.storage.zioy_biubiubiu_flag//&&lib.filter.targetEnabled2(event.card,event.player,player);
                                },
                                direct:true,
                                content:function(){
                                    'step 0'
                                    player.chooseTarget(get.prompt2('zioy_biubiubiu').split('②')[1],function(card,player,target){
                                    ...
                                    if(result.bool){
                                        ...
                                        if(event.card.name == 'wuxie'){
											...
											.set('ai',target=>{
												if(typeof lib.card[event.card.name].ai.result.target == 'function')return lib.card[event.card.name].ai.result.target(player,target)
											});
                                            event.skip = false;
                                        }else{
											game.log(player,'尝试执行',event.card,'失败')
				...
				translate: {
					...
					"zioy_shulin": "倏翷",
					"zioy_shulin_info":
						"觉醒技，每局游戏限2次。出牌阶段开始时，你可以回复所有已损失体力并进入觉醒状态。再次使用此技能时回复X点体力并将多出的回复值转化为摸牌数，然后你退出觉醒状态（X为觉醒状态持续时间）",
					"zioy_huangyi": "煌熠",
					"zioy_huangyi_info":
					...
					"zioy_f42chongzai_info":
						"出牌阶段各限1次：<br>①若你为“驭械”状态，你令所有角色受到1点无来源伤害，然后你摸3张牌，然后若你未损失体力则储存1点体力，若你拥有护甲则储存1点护甲。<br>②若你不为“驭械”状态，你可以弃置至少2张花色各不同的手牌，根据你弃置的手牌数执行以下效果：<br>不为4张：你获得等同于弃置手牌数-1枚“能量”标记。<br>4张：你获得4枚“能量”标记。<br>然后将当前体力值超过1的部分转化为等量“能量”标记，之后释放你储存的体力值。使用此技能后的每个的回合结束阶段，根据你的“能量”标记数量，执行以下效果：<br>小于体力上限：你获得1枚“能量”标记。<br>不小于体力上限：你弃置所有“能量”标记，获得等量护甲并释放储存的护甲，进入“驭械”状态，若你以此法获得的护甲值大于你的体力上限，你获得1点体力上限。",
					"zioy_yuemai": "岳衇",
					"zioy_yuemai_info":
					...
					"zioy_biubiubiu": "Biu! ",
					"zioy_biubiubiu_info": "①一名未拥有“泡泡”的角色使用非转化的普通锦囊牌结算后，你将此牌置于其武将牌上，称为“泡泡”。<br>②当你成为/使一名角色成为一张牌的唯一目标时，你选择一名拥有“泡泡”的角色，弃置其“泡泡”并无视距离与时机尝试执行其中内容，然后与其交换座位并令其代替你进行此牌后续结算，若你有“泡泡”则令其获得之。"
				}
			},
			intro: "??????????????????????????<br>拒绝规范描述",
			author: "喵喵",
